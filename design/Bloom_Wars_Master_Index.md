...
**Current build status, as of this session (25 Aug 2026):** all 36 missions of The Amaranth Reckoning are built, sim-tuned, tested, and delivered — Act I (Missions 1–12), Act II (13–24), and Act III (25–36, including the finale and its final boss "The Cradle"). The campaign is complete. On top of that, the live "mission commander down" fix landed the same day (Rourke reaching 0 HP is meant to void the mission attempt and return the player to the briefing screen, rather than the old silent-restock bug — see `claude/build_log/engine_systems/permadeath_and_commander_down.md`) — **but see the priority queue below: this has since regressed and isn't currently confirmed working.**

## Known issues — priority queue

- **Commander down appears broken again, found 25 Aug 2026 (later the same day it was fixed).** `commanderDown.test.ts` was a clean 536/536 when built; now 2 of its 7 tests fail. Diagnosis so far points at combat resolution (a rigged guaranteed-kill attack no longer downs the target at all), not the exemption logic itself, which still reads correct on inspection. Not root-caused or fixed — Maxime's own call: "keep a prio note of it, ill get to it when I need to work on the battle part of the game." Full write-up: `claude/build_log/engine_systems/permadeath_and_commander_down.md` and `claude/Bloom_Wars_Walkable_Hub_Build_Plan_v1.md` §8.
