...
**Current build status, as of this session (25 Aug 2026):** all 36 missions of The Amaranth Reckoning are built, sim-tuned, tested, and delivered — Act I (Missions 1–12), Act II (13–24), and Act III (25–36, including the finale and its final boss "The Unnamed"). The campaign is complete. On top of that, the live "mission commander down" fix landed the same day (Rourke reaching 0 HP is meant to void the mission attempt and return the player to the briefing screen, rather than the old silent-restock bug — see `claude/build_log/engine_systems/permadeath_and_commander_down.md`) — **but see the priority queue below: this has since regressed and isn't currently confirmed working.**

## Known issues — priority queue

- **Commander down appears broken again, found 25 Aug 2026 (later the same day it was fixed) — sharpened, same day: it's flaky, not dead, and the likely cause is identified.** `commanderDown.test.ts` was a clean 536/536 when built; re-running it repeatedly now shows it passing clean some runs and failing 1–2 of 7 other runs — not a consistent break. Root cause, read from the actual code rather than guessed: the test's `riggedHostileAttack()` helper forces the attacker's damage and the target's HP but never touches the roll at `engine/mission.ts`'s `rollMeepsDodge()` — the Meeps dodge house rule (House rule #1b, added 23 Aug 2026, *after* this test suite was last verified clean) gives Rourke, a Meeps pilot, a `MEEPS_DODGE_CHANCE` chance to dodge any non-Tank-sourced hit outright, including this rigged one, since the test's attacking hostile (`bloom_crawlmass`) isn't Tank-path. When she dodges, HP never reaches 0, `handleDowned()` never fires, and the assertions fail — same runs where she doesn't dodge, the suite passes. Not the exemption logic, and not something in the Hub-scene work either. Still not fixed — Maxime's own call stands: "keep a prio note of it, ill get to it when I need to work on the battle part of the game." Likely fix, whenever he gets there: make `riggedHostileAttack()` use a Tank-path hostile (dodge-exempt by the same house rule) or explicitly stub the dodge roll — a test-helper fix, not an engine one, as far as this diagnosis can tell. Full write-up: `claude/build_log/engine_systems/permadeath_and_commander_down.md` and `claude/Bloom_Wars_Walkable_Hub_Build_Plan_v1.md` §8–§9.
