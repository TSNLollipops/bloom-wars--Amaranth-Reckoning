...
## 9. The Heirlooms — Gjallar, and Nineteen More
...
**SUPERSEDED IN PART, 2 Sep 2026 — read this before using the table below.** The twenty-entry pool here is the ORIGINAL brainstorm and is kept as that: a menu of ideas, not the shipped design. `Bloom_Wars_Heirloom_Weapons_Plan_v1.md` (28-29 Aug 2026) cut the pool to **ten**, restructured each one into three independently-rankable abilities, and replaced the "reach A tier and survive a beat" unlock with a company-points recruitment shortlist. Those ten are now real, typed, tested data in `src/data/heirlooms.ts`, which is ground truth for anything about an Heirloom's actual kit — this table is not.

Several of the ten grew out of rows below (Widow's Ledger, The Last Word, Deadfall, The Iron Oath, Salt the Root, Cinder Line, The Cutting Room, Farsight's Reckoning), but the mapping is deliberately NOT written out here, because it isn't clean: the shipped versions were redesigned, not renamed, and one of them (The Stolen Seal) draws on two different rows at once. Forcing a row-by-row correspondence would invent a history that didn't happen.

**The ten also got their own names, 2 Sep 2026** — Maxime: *"give it an epic fantasy sounding name. relate to its action, can be in another language. like zaibatsu."* Each Heirloom now carries a grand name in a real language chosen for what the weapon does, with the old design-doc title kept alongside it as an epithet (`HeirloomDef.epithet`): **Gjallar**, **Simulacrum** *(The Stolen Seal)*, **Skuld** *(Widow's Ledger)*, **Vindex** *(The Iron Oath)*, **Ichigeki** *(Deadfall)*, **Migawari** *(The Last Word)*, **Delenda** *(Salt the Root)*, **Zanretsu** *(The Cutting Room)*, **Surtr** *(Cinder Line)*, **Panoptes** *(Farsight's Reckoning)*. See `Bloom_Wars_Heirloom_Naming_Update_2Sep2026.md` for the reasoning behind each.

| Name | Best fit | Effect & cost |
|---|---|---|
| **Gjallar** *(named 2 Sep 2026; this row's old title, "Requiem", was promoted to the name of the SYSTEM — see the note above)* | Party-founding — Bosk's, then Rourke's | A line of damage eight tiles long, friend and foe alike, no exception. Ignores the full-HP cap. Cost: it doesn't check sides — a bad line costs you the ally standing in it. |
