...
**Status update, 25 August 2026 (superseding the 22 Aug note below — that note was left stale for several days and caused real confusion in at least one other conversation working from this doc; if you're reading this to answer "what's actually built," this paragraph is the one to trust, not the one below it).** Missions 1–32 are built, sim-tuned, tested, and delivered as of this session: Act I (Missions 1–12), Act II (13–24), and Act III's first 8 missions (25–32). Missions 33–36 — the campaign's actual finale ("The Innermost Ring," "No Word from the Fleet," "The Last Ring" with the new final-boss archetype "The Cradle," and "Until Relief") — are in progress this same session: The Cradle's stat block is implemented in `data/bloom.ts`, but the four mission definitions, maps, and sim-tuning are not built yet. `src/data/campaignAmaranth.ts` is ground truth for exact current state — check it directly rather than trusting any status paragraph, including this one, to have been kept current. `claude/Bloom_Wars_Amaranth_Act1_Build_Log_v1.md` has the batch-by-batch build history and final tuning numbers for every mission built so far. Team One's book-canon 4-mission slice remains archived in the repo (`src/data/campaign.ts`/`maps.ts`), not deleted, not currently shipped.
...
**Status, 25 Aug 2026 (see the top-of-document status update for the authoritative version — this line is left for historical continuity):** Missions 1–32 across Acts I and II and Act III's first 8 missions are built in the repo (`src/data/campaignAmaranth.ts`, `mapsAmaranth.ts`), playable from mission-select. Missions 33–36 (the finale) are in progress this same session — see the top-of-document status update.
...
**Flags a real mismatch, not resolved here.** The sandbox's own approximation of this rule (`Bloom_Wars_Antfarm_Carrier_Hub_v1.md` §13.2) currently implements the older framing — a death roll landing on her redirects to whoever she's paired with, so someone else dies in her place instead. Under this sharper understanding that's not quite right: the real design has nobody dying when she'd go down, the mission just fails and resets. Flagged and reasoned through in the Antfarm doc's own dated log rather than duplicated or silently rebuilt here — not fixed yet, since it turns out to touch more than one place in the sandbox's code.
...
- **The Cradle** (Act 3 final threat) — the source, growing beneath Meridian, emergent-tier, the campaign's true final boss, largest Endurance wall in the campaign, built to make the Collapse rule do maximum horror work. **Archetype built 25 Aug 2026, `bloom_cradle` (`data/bloom.ts`) — a real new stat block, not a Heartwood reuse: END 560 (exceeds Heartwood's 400, per this section's own "largest Endurance wall" spec), VIT 70, attackRange [1,5], attackPower 75. See that archetype's own comment for the full derivation. Mission 35 itself — where this archetype actually gets used — is not built yet as of 25 Aug 2026; once it is, its own comment in `campaignAmaranth.ts` will cover the hold_zone-not-eliminate_all interpretation call.**
...
| **Salt the Root** | Anti-Bloom specialist | Massive bonus damage against sessile/hive-type Bloom specifically (Gallcyst-family, the Wellroot, the Cradle). Cost: notably weak against everything else. |
...
26. **The Cradle Beneath** — *Extract Unit.* Discovery of the hive beneath Meridian, fed by the Wellroot network.
...
35. **The Last Ring** — *Hold Zone* [final boss breaches]. The Cradle begins breaching into the open mid-siege.
...
**Build status, 25 Aug 2026: Missions 33–36 are in progress this session, not yet built.** The Cradle's archetype (stats below in §8) is implemented in `data/bloom.ts`. The four mission definitions, their maps, and their sim-tuning are not started yet. Once they land, this line will point to the build log's own batch-7 addendum for the sim-tested numbers, the interpretation calls made building the finale, and The Cradle's stat derivation — check that addendum (or `campaignAmaranth.ts` directly) for the real current state rather than trusting this line to have been kept current.
